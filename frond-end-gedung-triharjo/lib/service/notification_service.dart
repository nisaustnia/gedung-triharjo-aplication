...
class NotificationService {
  static Future<List<NotificationModel>> fetchListNotificationData() async {
    String? token = await getToken();
    try {
      final dio = Dio();
      final response = await dio.get("$triharjoAPI/notif",
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load List Notification Data');
      }
    } catch (error) {
      log("$error");
